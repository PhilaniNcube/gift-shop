  ...
  return (
    <Layout>
      <div>
        ...
        <form
          className="p-8 border-spacing-3 border border-dashed rounded-lg border-slate-500 flex gap-6"
        ...
        >
          ...
          <div className="flex-1">
            <img src={category.image.src} alt="Image" className="w-[500px] aspect-[16/9] object-cover" />
          </div>
        </form>
        <form
...
export async function getServerSideProps(ctx: GetServerSidePropsContext) {
  ...
  const { data: category } = await supabaseServerClient
    .from("categories")
    .select("*").eq('slug', ctx.query.slug).single();
